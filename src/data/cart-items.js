export const cartItems = [
  {
    id: 0,
    features: [
      {
        id: 1,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 2,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 3,
        icon: "/icon-three.svg",
        title: "feature three",
      },
      {
        id: 4,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 5,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 6,
        icon: "/icon-three.svg",
        title: "feature three",
      },
    ],
    isFavorite: false,
    image: "/cart-item.png",
    description: {
      name: "Just Milk Semi-Skimmed 6 x 1L (Case of 2)",
      price: 1.59,
      discount: null,
      inCartAmmount: 2,
      special: null,
      isPriceChanged: false,
      img: "/main-item.png",
    },
    relatedProducts: [
      {
        id: 10,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 11,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
      {
        id: 12,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 13,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
    ],
    composition: {
      nutrition: {
        description:
          "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
        points: [
          "Lorem ipsum dolor sit amet",
          "Consectetur adipiscing elit",
          "Ultrices at egestas tempus",
          "Duis egestas semper dolor",
        ],
        calories: [
          {
            name: "Kkal",
            value: 28,
            ri: 4,
          },
          {
            name: "Proteins",
            value: 1.3,
            ri: 10,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
          {
            name: "Niacin",
            value: 6.4,
            ri: 40,
          },
          {
            name: "Vitamin B6",
            value: 0.8,
            ri: 57,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
        ],
      },
      ingredients:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
      other: [
        {
          name: "Страна производства",
          value: "Россия",
        },
        {
          name: "Состав",
          value: "цельное молока",
        },
        {
          name: "Срок годности",
          value: "12 дн.",
        },
        {
          name: "Изготовитель",
          value: "ОАО Компания Юнимилк",
        },
        {
          name: "Жирность",
          value: "4.5%",
        },
        {
          name: "Содержание лактозы",
          value: "обычное (более 1% лактозы)",
        },
        {
          name: "Витаминизированное/обогащенное",
          value: "нет",
        },
      ],
    },
  },
  {
    id: 1,
    features: [
      {
        id: 1,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 2,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 3,
        icon: "/icon-three.svg",
        title: "feature three",
      },
      {
        id: 4,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 5,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 6,
        icon: "/icon-three.svg",
        title: "feature three",
      },
    ],
    isFavorite: false,
    image: "/cart-item.png",
    description: {
      name: "Just Milk Semi-Skimmed 6 x 1L (Case of 2)",
      price: 1.59,
      discount: 1.59,
      inCartAmmount: 2,
      isPriceChanged: true,
      special: "Price has been changed: <span style='color: black; text-decoration: line-through; font-weight: 700'>(£2.59)<span>",
      img: "/main-item.png",
    },
    relatedProducts: [
      {
        id: 10,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 11,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
      {
        id: 12,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 13,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
    ],
    composition: {
      nutrition: {
        description:
          "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
        points: [
          "Lorem ipsum dolor sit amet",
          "Consectetur adipiscing elit",
          "Ultrices at egestas tempus",
          "Duis egestas semper dolor",
        ],
        calories: [
          {
            name: "Kkal",
            value: 28,
            ri: 4,
          },
          {
            name: "Proteins",
            value: 1.3,
            ri: 10,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
          {
            name: "Niacin",
            value: 6.4,
            ri: 40,
          },
          {
            name: "Vitamin B6",
            value: 0.8,
            ri: 57,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
        ],
      },
      ingredients:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
      other: [
        {
          name: "Страна производства",
          value: "Россия",
        },
        {
          name: "Состав",
          value: "цельное молока",
        },
        {
          name: "Срок годности",
          value: "12 дн.",
        },
        {
          name: "Изготовитель",
          value: "ОАО Компания Юнимилк",
        },
        {
          name: "Жирность",
          value: "4.5%",
        },
        {
          name: "Содержание лактозы",
          value: "обычное (более 1% лактозы)",
        },
        {
          name: "Витаминизированное/обогащенное",
          value: "нет",
        },
      ],
    },
  },
  {
    id: 2,
    features: [
      {
        id: 1,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 2,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 3,
        icon: "/icon-three.svg",
        title: "feature three",
      },
      {
        id: 4,
        icon: "/icon-one.svg",
        title: "feature one",
      },
      {
        id: 5,
        icon: "/icon-two.svg",
        title: "feature two",
      },
      {
        id: 6,
        icon: "/icon-three.svg",
        title: "feature three",
      },
    ],
    isFavorite: false,
    image: "/cart-item.png",
    description: {
      name: "Just Milk Semi-Skimmed 6 x 1L (Case of 2)",
      price: 1.59,
      discount: 1.19,
      isPriceChanged: true,
      inCartAmmount: 0,
      special: "Special offer: <span style='color: black; font-weight: 700'>-20%<span>",
      img: "/main-item.png",
    },
    relatedProducts: [
      {
        id: 10,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 11,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
      {
        id: 12,
        name: "Best-in 10 British Big Eggs",
        img: "item-one.png",
        price: 1.59,
        isFavorite: true,
      },
      {
        id: 13,
        name: "Best-in 10 British Big Eggs",
        img: "item-two.png",
        price: 1.59,
        isFavorite: false,
      },
    ],
    composition: {
      nutrition: {
        description:
          "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
        points: [
          "Lorem ipsum dolor sit amet",
          "Consectetur adipiscing elit",
          "Ultrices at egestas tempus",
          "Duis egestas semper dolor",
        ],
        calories: [
          {
            name: "Kkal",
            value: 28,
            ri: 4,
          },
          {
            name: "Proteins",
            value: 1.3,
            ri: 10,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
          {
            name: "Niacin",
            value: 6.4,
            ri: 40,
          },
          {
            name: "Vitamin B6",
            value: 0.8,
            ri: 57,
          },
          {
            name: "Fats",
            value: 0,
            ri: 12,
          },
          {
            name: "Carbs",
            value: 28,
            ri: 10,
          },
        ],
      },
      ingredients:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ultrices at egestas tempus duis egestas semper dolor morbi purus. ",
      other: [
        {
          name: "Страна производства",
          value: "Россия",
        },
        {
          name: "Состав",
          value: "цельное молока",
        },
        {
          name: "Срок годности",
          value: "12 дн.",
        },
        {
          name: "Изготовитель",
          value: "ОАО Компания Юнимилк",
        },
        {
          name: "Жирность",
          value: "4.5%",
        },
        {
          name: "Содержание лактозы",
          value: "обычное (более 1% лактозы)",
        },
        {
          name: "Витаминизированное/обогащенное",
          value: "нет",
        },
      ],
    },
  },
];
